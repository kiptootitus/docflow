...
import { Plus, Search, Send, Download, Copy, Trash2, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link to="/invoices/new" className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors">
          ...
          <tbody className="divide-y divide-gray-50">
            ...
            {invoices.map((inv) => (
              <tr key={inv.id} className="hover:bg-gray-50 transition-colors group">
                ...
                <td className="px-6 py-4 text-sm text-gray-700">{inv.client_name ?? <span className="text-gray-300">—</span>}</td>
                <td className="px-6 py-4 text-right text-sm font-semibold text-gray-900">
                  {formatCurrency(inv.total_amount, inv.currency)}
                </td>
                <td className="px-6 py-4 text-center">
                  <span className={cn("text-xs font-medium px-2.5 py-1 rounded-full capitalize", STATUS_COLORS[inv.status])}>
                    {inv.status}
                  </span>
                </td>
